...
- **2026-06-19 — Added 4 mid/late parkour checkpoints.** The back half of the climb (x≈232→590, father→friend→dog/house) previously had **zero** checkpoints; all 3 existing photos sat at x≤232. Duplicated the `CheckpointPhoto` prefab instance into 4 fitting spots, each verified to sit ~1.0–1.1m above a solid (non-moving) Cube platform: `CheckpointPhoto_ReachedFather` (328,45.1,217) on Cube(8) — after the father section; `CheckpointPhoto_Friend` (425,43.5,210) on Cube(13) — friend stretch; `CheckpointPhoto_Descent` (490,24.3,210) on Cube(17) — bottom of the big drop (most punishing fall to recover); `CheckpointPhoto_HouseApproach` (560,43.6,210) on Cube(20) — final climb to dog/house. Their respawn-point children were placed on each platform surface. `data` set to **null** on all 4 so they act as pure respawn points (the `CheckpointUI` toast still fires; no narrative voice line replays). Boost-gating checked: all five current zones are **Speed** type (no jump-gating left), and the pre-existing checkpoints already sit inside `WholeParkourBoost`, so a speed-zone checkpoint matches established convention and a respawn there is still passable at normal speed. Subway_ender.unity, saved.
